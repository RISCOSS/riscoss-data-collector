...
public class RDCSonar implements RDC {
	...
	static {
		...
		parameterMap.put( "Sonar_resourceKey", new RDCParameter( "Sonar_resourceKey", "", "org.ow2.bonita:bonita-server", null ) );
		parameterMap.put( "Sonar_host", new RDCParameter( "Sonar_host", "", "org.ow2.bonita:bonita-server", null ) );
		parameterMap.put( "Sonar_singleMetrics", new RDCParameter( "Sonar_singleMetrics", "", "", null ) );
		parameterMap.put( "Sonar_historyMetrics", new RDCParameter( "Sonar_historyMetrics", "", "", null ) );
		parameterMap.put( "Sonar_by_file_Metrics", new RDCParameter( "Sonar_by_file_Metrics", "", "", null ) );
	}
	...
	class FormattedParameters {
		...
		public FormattedParameters( Map<String, String> map ) {
			try {
				...
				singleMetrics = map.get("Sonar_singleMetrics").split("\\s*\\,\\s*");
				historyMetrics = map.get("Sonar_historyMetrics").split("\\s*\\,\\s*");
				String fileMetricString = map.get( "" );
				if( fileMetricString == null ) fileMetricString = "ncloc, complexity";
	...
	public Map<String, RiskData> getIndicators( String target ) {
		...
		if( !params.isValid() ) return map;
		
		//retrieve data for single values and for history
		SonarStatistics statistics = getStatistics( params );
		//retrieve data per class
		ArrayList<ClassInfo> classvalueslist = ClassInfoManager.getStatisticsPerClass( params.host, params.resourceKey, params.fileMetrics);
		//elaborate per-class data to get the needed indicators
		
		//*****************************
		//calculate specific indicators
		SonarQuality.createIndicators(statistics, params.fileMetrics, classvalueslist);
		//*****************************
		
		if (statistics != null) {
			for (String key : statistics.singleValues.keySet()) {
				RiskData rd = new RiskData( "Sonar " + key, target, new Date(), RiskDataType.NUMBER, statistics.singleValues.get(key) );
...
//				map.put("Sonar " + key, statistics.singleValues.get(key));
			...
			for (String key : statistics.historyValues.keySet()) {
				...
				d.setValues(l);
				RiskData rd = new RiskData( "Sonar History " + key, target, new Date(), RiskDataType.DISTRIBUTION, d );
	 ...
	 */
		protected SonarStatistics getStatistics( FormattedParameters params ) {
			...
			for (String m : params.singleMetrics) {
				...
				RetrieveStats.getStoreCurrentValue( params.host, params.resourceKey, m, stats);
			...
			for (String m : params.historyMetrics) {
				...
				if ((val = RetrieveStats.getHistoricValues( params.host, params.resourceKey, m, from, to))!=null)
